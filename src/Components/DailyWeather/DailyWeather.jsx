        ...
        {daily.time.map((timestamp, i) => {
          ...
          return (
            <div style={{ background: `linear-gradient(${daily.weather_code[i] > 50 ? "DodgerBlue,lightblue" : "yellow,lightyellow"})` }} key={timestamp} onClick={() => setSelectedDay(i)} className={i === selectedDay ? "SelectedDailyWeatherItem" : "dailyWeatherItem"}>
              ...
              <div className="temps">
                <p><span>{t.max}:</span> {degree === "Цельсий" ? daily.temperature_2m_max[i] + "°" : Math.round(daily.temperature_2m_max[i] * 9 / 5 + 32) + "F"}</p>
                <p><span>{t.min}:</span> {degree === "Цельсий" ? daily.temperature_2m_min[i] + "°" : Math.round(daily.temperature_2m_min[i] * 9 / 5 + 32) + "F"}</p>
              </div>
            </div>
